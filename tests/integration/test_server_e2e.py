...
def log_output(pipe: Any, log_func: Any) -> None:
    """Log output from pipe line by line."""
    for line in iter(pipe.readline, ""):
        log_func(line.strip())


def start_server() -> subprocess.Popen[str]:
    """Start the FastAPI application in a background subprocess."""
    command = [
        sys.executable,
        "-m",
        "uvicorn",
...
def wait_for_server(timeout: int = 90, interval: int = 1) -> bool:
    """Wait for agent card endpoint to respond HTTP 200."""
...
def server_fixture(request: Any) -> Iterator[subprocess.Popen[str]]:
    """Pytest fixture managing background uvicorn server lifecycle."""
...
def test_agent_card(server_fixture: subprocess.Popen[str]) -> None:
    """Test that A2A Agent Card endpoint is accessible and valid."""
    response = requests.get(AGENT_CARD_URL, timeout=10)
...
def test_collect_feedback(server_fixture: subprocess.Popen[str]) -> None:
    ...
    feedback_data = {
        ...
        "text": "Excellent inventory alert functionality!",
    }
